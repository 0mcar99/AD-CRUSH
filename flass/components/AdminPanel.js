...
export default function AdminPanel({ onClose }) {
  ...
  const handleDeleteUser = async (email) => {
    ...
    const confirmDelete = window.confirm(
      `⚠️ WARNING: Are you sure you want to permanently delete all data for ${email}?\n\nThis will permanently delete:\n1. Their login credentials/profile\n2. All of their campaign submissions\n3. All chat history\n4. Newsletter subscription\n\nThis action cannot be undone.`
    );
    ...
    try {
      ...
      if (res.ok) {
        alert(`Successfully deleted all data for ${email}`);
        setSelectedEmail("");
        // Remove from the local users list
        setUsers((prev) => prev.filter((u) => u.email !== email));
      } else {
        const errData = await res.json();
        alert(errData.error || "Failed to delete user data.");
      }
    } catch (err) {
      console.error(err);
            ...
            {selectedEmail ? (
              <div className={styles.mainContent}>
                ...
                <div className={styles.profileHeader}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", flexWrap: "wrap", gap: "12px", marginBottom: "12px" }}>
                    ...
                    <button className={styles.deleteUserBtn} onClick={() => handleDeleteUser(selectedUser?.email)}>
                  ...
                  <div className={styles.profileMeta}>
                    ...
                    <div className={styles.profileSubGrid}>
                      ...
                      <span><strong>Registered:</strong> {selectedUser?.createdAt ? new Date(selectedUser.createdAt).toLocaleString() : "—"}</span>
                      <span><strong>Subscribed:</strong> {selectedUser?.subscribed ? "Yes ✅" : "No ❌"}</span>
                    </div>
                  </div>
                </div>
